...
function AuthPage() {
  ...
  async function handleVerifyCode(e: React.FormEvent) {
    ...
    try {
      ...
      const token = z.string().trim().regex(/^\d{6}$/, "Enter the 6-digit code").parse(code);
      ...
      <div className="card-felt shadow-card rounded-2xl p-6">
        ...
        {magicSent ? (
          <div className="rounded-lg border border-border bg-background/40 p-4 text-center text-sm">
            ...
            <button className="mt-3 text-gold hover:underline" onClick={() => setMagicSent(false)}>
          ...
          <form onSubmit={handleVerifyCode} className="space-y-3">
            <div>
              ...
              <Input id="email-code" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required maxLength={255} />
            </div>
            <div>
            ...
            <button type="button" className="w-full text-xs text-muted-foreground hover:underline" onClick={() => setCodeMode(false)}>
              Back to magic link
            </button>
          </form>
        ) : (
          <form onSubmit={handleMagicLink} className="space-y-3">
            ...
            <button type="button" className="w-full text-xs text-muted-foreground hover:underline" onClick={() => setCodeMode(true)}>
