...
import { AWARDS, fetchAllResults, fetchNights, fetchProfiles, formatMoney, formatEUDate, formatDisplayName } from "@/lib/poker";
...
function Player() {
  ...
  useEffect(() => {
    if (profile) { setName(profile.name); setNickname(profile.nickname ?? ""); setAvatar(profile.avatar_url ?? ""); }
  }, [profile]);
  ...
  useEffect(() => {
    ...
    async function resolve() {
      if (!avatar) { setAvatarDisplay(""); return; }
      if (/^https?:\/\//i.test(avatar)) { setAvatarDisplay(avatar); return; }
      const { data } = await supabase.storage.from("avatars").createSignedUrl(avatar, 60 * 60 * 24 * 7);
  ...
  async function uploadAvatar(file: File) {
    ...
    try {
      ...
      const { error: updErr } = await supabase.from("profiles").update({ avatar_url: path }).eq("id", me);
  ...
  const mine = useMemo(() => (results.data ?? []).filter((r) => r.user_id === id), [results.data, id]);
  ...
  async function save() {
    const { error } = await supabase.from("profiles").update({
      name: name.trim(), nickname: nickname.trim() || null, avatar_url: avatar.trim() || null,
    }).eq("id", id);
  ...
  return (
    <AppShell>
      <div className="mb-6 flex items-center gap-4">
        <div className="chip-ring h-16 w-16 shrink-0 rounded-full shadow-gold" style={avatarDisplay ? { backgroundImage: `url(${avatarDisplay})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined} />
        <div className="flex-1">
          <h1 className="font-display text-3xl font-bold">{formatDisplayName(profile?.name, profile?.nickname)}</h1>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <div className="card-felt shadow-card rounded-2xl p-5 lg:col-span-2">
          ...
          {mine.length === 0 ? (
            <div className="rounded-md border border-dashed border-border/60 p-4 text-center text-sm text-muted-foreground">No results yet.</div>
          ) : (
            <ul className="space-y-2 text-sm">
              {mine.slice(-10).reverse().map((r) => {
        ...
        <div className="card-felt shadow-card rounded-2xl p-5">
          <div className="mb-3 flex items-center gap-2 font-display text-lg font-semibold"><Award className="h-4 w-4 text-gold"/>Awards</div>
          {awards.length === 0 ? (
      ...
      {canEditName && (
        <div className="card-felt shadow-card mt-6 rounded-2xl p-5">
          ...
          <div className="mb-4 flex items-center gap-4">
            <div className="h-20 w-20 shrink-0 rounded-full border border-border/60 bg-background/40" style={avatarDisplay ? { backgroundImage: `url(${avatarDisplay})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined} />
              ...
              <div className="flex items-center gap-2">
                <Button asChild size="sm" variant="outline" disabled={uploading}>
                  <label className="cursor-pointer">
                    ...
                    <input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadAvatar(f); e.currentTarget.value = ""; }} />
                  </label>
                </Button>
                {avatar && (
          ...
          <Button className="mt-4 bg-gold shadow-gold" onClick={save}>Save profile</Button>
        </div>
      )}
      {isMe && (
        <div className="card-felt shadow-card mt-6 rounded-2xl">
          ...
          >
            ...
            <ChevronDown className={`h-4 w-4 transition-transform ${showNotifSettings ? "rotate-180" : ""}`} />
          </button>
          {showNotifSettings && (
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "up"|"down" }) {
  return (
    <div className="card-felt rounded-2xl p-4">
      ...
      <div className={"mt-1 font-mono text-xl " + (tone === "up" ? "text-emerald-400" : tone === "down" ? "text-red-400" : "text-foreground")}>{value}</div>
    </div>
  );
}