  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      return (nights ?? []).map((n) => {
        ...
        const nets = rows.map((r) => ({ name: r.player_name, user_id: r.user_id, net: Number(r.net_result) }));
  ...
  return (
    <AppShell>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Fewest transfers needed to settle up each completed night.</p>
      </div>
            ...
            <div key={night.id} className="card-felt shadow-card rounded-2xl p-5">
              ...
              {Math.abs(imbalance) > 0.5 && (
                <div className="mb-3 rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-300">
                  Table doesn't balance ({formatMoney(imbalance, night.currency)}). Settlement is approximate.
                </div>
              )}
                ...
                <ol className="space-y-1 text-sm">
                  {transfers.map((t, i) => (
                    <li key={i} className="flex items-center justify-between rounded-md bg-background/30 px-3 py-2">
                      ...
                      <div className="font-mono text-gold">{formatMoney(t.amount, night.currency)}</div>
                    </li>
                  ))}
                </ol>
              )}
