...
import { createPokerTable, listMyPokerTables, listMyEndedPokerTables, getMyWallet, topUpWallet, listMyWalletTransactions } from "@/lib/poker-table.functions";
import { createBlackjackTable, listMyBlackjackTables, rematchBlackjackTable } from "@/lib/blackjack.functions";
...
function PlayLobby() {
  ...
  const bjEnded = useQuery({ queryKey: ["bj-tables-ended"], queryFn: () => listBjEndedFn() as any });
  ...
  const withdrawMut = useMutation({
    mutationFn: (amount: number) => withdrawFn({ data: { amount } }) as Promise<{ chips: number; locked: number; available: number; amount: number }>,
    onSuccess: (res) => {
      qc.invalidateQueries({ queryKey: ["poker-wallet"] });
      qc.invalidateQueries({ queryKey: ["poker-wallet-tx"] });
      qc.invalidateQueries({ queryKey: ["my-settlements"] });
      toast.success(`Cashed out ${res.amount.toLocaleString()} chips. Balance: ${res.chips.toLocaleString()}.`);
      setWithdrawAmount("");
    },
  ...
  const rematchPokerMut = useMutation({
    mutationFn: (table_id: string) => rematchPokerFn({ data: { table_id } }) as Promise<{ id: string }>,
    onSuccess: (res) => {
  ...
  const rematchBjMut = useMutation({
    mutationFn: (table_id: string) => rematchBjFn({ data: { table_id } }) as Promise<{ id: string }>,
    onSuccess: (res) => {
      qc.invalidateQueries({ queryKey: ["bj-tables"] });
      ...
      <div className="mx-auto max-w-4xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <Dialog open={cashierOpen} onOpenChange={setCashierOpen}>
              <DialogTrigger asChild>
                <Button size="sm" variant="secondary">Cashier</Button>
              </DialogTrigger>
              <DialogContent className="max-h-[90vh] overflow-y-auto">
                <DialogHeader><DialogTitle>Cashier</DialogTitle></DialogHeader>
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-amber-500/30 bg-gradient-to-br from-background/80 to-background/40 p-4">
                    <div className="text-xs uppercase tracking-wide text-muted-foreground">Locked</div>
                    <div className="mt-1 flex items-baseline gap-2">
                  ...
                  <div className="rounded-xl border border-emerald-500/30 bg-gradient-to-br from-background/80 to-background/40 p-4">
                    <div className="text-xs uppercase tracking-wide text-muted-foreground">Available to withdraw</div>
                    <div className="mt-1 flex items-baseline gap-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Chips are used for table buy-ins. When you leave a table your remaining stack returns to your wallet.
                </p>
                <div>
                  <Label className="text-xs uppercase tracking-wide text-muted-foreground">Quick add</Label>
                ...
                <div>
                  <Label htmlFor="deposit" className="text-xs uppercase tracking-wide text-muted-foreground">Custom amount</Label>
                  <Input id="deposit" type="number" min={1} max={1000000} value={depositAmount} onChange={(e) => setDepositAmount(e.target.value)} />
                </div>
                {(() => {
                  ...
                  return (
                    <div className="rounded-md border border-emerald-500/30 bg-emerald-500/5 p-3">
                      <div className="flex items-center justify-between">
                        <Label className="text-xs uppercase tracking-wide text-emerald-200">Cash out chips</Label>
                        <span className="text-[10px] text-muted-foreground">Max {max.toLocaleString()}</span>
                      </div>
                      <p className="mt-1 text-[11px] text-muted-foreground">
                        Removes chips from your wallet. {locked > 0 ? `${locked.toLocaleString()} chips are locked until pending settlements are confirmed. ` : ""}
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    <Label className="text-xs uppercase tracking-wide text-muted-foreground">Recent activity</Label>
                  ...
                  <div className="max-h-56 overflow-y-auto rounded-md border border-border/60 bg-background/40">
                    ...
                    {txs.data?.map((t: any) => {
                      ...
                      const label =
                        t.kind === "deposit" ? "Added chips" :
                        t.kind === "buy_in" ? "Table buy-in" :
                        t.kind === "cashout" ? "Left table" :
                        t.kind === "settlement" ? "Settlement" :
                        t.kind === "settlement_confirmed" ? "Settlement received" :
                        t.kind === "withdrawal" ? "Cashed out" :
                        t.kind === "rebuy" ? "Rebuy" : "Adjustment";
                      return (
                        <div key={t.id} className="flex items-center justify-between border-b border-border/40 px-3 py-2 last:border-0">
                          <div className="min-w-0">
                            <div className="text-sm">{label}</div>
                            {t.note && <div className="truncate text-[10px] text-muted-foreground">{t.note}</div>}
                            <div className="text-[10px] text-muted-foreground">
                              {new Date(t.created_at).toLocaleString()}
                            </div>
                          </div>
                          <div className="text-right">
                            <div className={`font-mono text-sm ${amt > 0 ? "text-emerald-400" : amt < 0 ? "text-red-400" : "text-muted-foreground"}`}>
                ...
                <DialogFooter>
                  <Button onClick={() => topUpMut.mutate(Number(depositAmount))} disabled={topUpMut.isPending || !Number(depositAmount)} className="bg-gold shadow-gold">
                    {topUpMut.isPending ? "Adding…" : `Add ${Number(depositAmount || 0).toLocaleString()} chips`}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
        ...
        <div className="mb-6">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button className="bg-gold shadow-gold"><Plus className="mr-1 h-4 w-4" /> New Poker Table</Button>
            </DialogTrigger>
            <DialogContent className="max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle>Create a table</DialogTitle></DialogHeader>
              <div className="space-y-3">
                <div>
                  <Label htmlFor="tname">Name</Label>
                  <Input id="tname" value={name} onChange={(e) => setName(e.target.value)} maxLength={80} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label htmlFor="sb">Small blind</Label>
                    <Input id="sb" type="number" step="0.5" min={0.5} value={sb} onChange={(e) => setSb(e.target.value)} />
                  </div>
                  <div>
                    <Label htmlFor="bb">Big blind</Label>
                    <Input id="bb" type="number" step="0.5" min={0.5} value={bb} onChange={(e) => setBb(e.target.value)} />
                  </div>
                  <div>
                    <Label htmlFor="buy">Buy-in (chips)</Label>
                    <Input id="buy" type="number" min={1} value={buyIn} onChange={(e) => setBuyIn(e.target.value)} />
                  </div>
                  <div>
                    <Label htmlFor="ms">Max seats</Label>
                    <Input id="ms" type="number" min={2} max={9} value={maxSeats} onChange={(e) => setMaxSeats(e.target.value)} />
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-1 grid gap-1 rounded-md border border-border/60 bg-background/30 p-2 max-h-48 overflow-auto">
                    {(profiles.data ?? []).map((p) => (
                      <label key={p.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-background/40">
                        <input type="checkbox" checked={invited.has(p.id)} onChange={() => toggle(p.id)} />
              ...
              <DialogFooter>
                ...
                <Button className="bg-gold shadow-gold" onClick={() => createMut.mutate()} disabled={createMut.isPending}>
          ...
          <Dialog open={bjOpen} onOpenChange={setBjOpen}>
            ...
            <DialogContent className="max-h-[90vh] overflow-y-auto">
              <DialogHeader><DialogTitle>Create a Blackjack table</DialogTitle></DialogHeader>
              <div className="space-y-3">
                <div>
                  <Label htmlFor="bjname">Name</Label>
                  <Input id="bjname" value={bjName} onChange={(e) => setBjName(e.target.value)} maxLength={80} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label htmlFor="bjmin">Min bet</Label>
                    <Input id="bjmin" type="number" min={1} value={bjMin} onChange={(e) => setBjMin(e.target.value)} />
                  </div>
                  <div>
                    <Label htmlFor="bjmax">Max bet</Label>
                    <Input id="bjmax" type="number" min={1} value={bjMax} onChange={(e) => setBjMax(e.target.value)} />
                  </div>
                </div>
                <div className="text-xs text-muted-foreground">
                  Choose the dealer for this table. Invitees can sit as players and bet from their wallets.
                </div>
                <div>
                  <Label>Dealer</Label>
                  <div className="mt-1 grid gap-1 rounded-md border border-border/60 bg-background/30 p-2 max-h-40 overflow-auto">
                    <label className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-background/40">
                      <input type="radio" name="bjdealer" checked={bjDealer === ""} onChange={() => setBjDealer("")} />
                      <span className="text-sm">Me (admin)</span>
                    </label>
                    {(profiles.data ?? []).map((p) => (
                      <label key={p.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-background/40">
                        <input type="radio" name="bjdealer" checked={bjDealer === p.id} onChange={() => setBjDealer(p.id)} />
                        <span className="text-sm">{formatDisplayName(p.name, p.nickname)}</span>
                      </label>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-1 grid gap-1 rounded-md border border-border/60 bg-background/30 p-2 max-h-48 overflow-auto">
                    {(profiles.data ?? []).map((p) => (
                      <label key={p.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-background/40">
                        <input type="checkbox" checked={bjInvited.has(p.id)} onChange={() => toggleBj(p.id)} />
                        <span className="text-sm">{formatDisplayName(p.name, p.nickname)}</span>
                      </label>
                    ))}
                  </div>
              ...
              <DialogFooter>
                <Button variant="ghost" onClick={() => setBjOpen(false)}>Cancel</Button>
                <Button className="bg-gold shadow-gold" onClick={() => createBjMut.mutate()} disabled={createBjMut.isPending}>
          ...
          <div className="mb-6">
            ...
            <div className="grid gap-3">
              {bjTables.data!.map((t: any) => (
                <Link key={t.id} to="/play/bj/$id" params={{ id: t.id }} className="card-felt shadow-card rounded-2xl p-4 block hover:ring-1 hover:ring-gold/40 transition">
                  <div className="flex items-center justify-between">
                    <div>
                    ...
                    <div className="text-right text-sm">
                      <div className="flex items-center gap-1"><Users className="h-4 w-4" /> {t.seated_count ?? 0}/6</div>
        ...
        <div className="grid gap-3">
          ...
          {tables.data?.map((t: any) => {
            ...
            return (
              <div key={t.id} className={`card-felt shadow-card rounded-2xl p-4 transition ${short ? "ring-1 ring-red-500/30" : "hover:ring-1 hover:ring-gold/40"}`}>
                <Link to="/play/$id" params={{ id: t.id }} className="block">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-display text-lg font-semibold">{t.name}</div>
                      <div className="text-xs text-muted-foreground">
                        Host: {t.host_name} · Blinds {t.small_blind}/{t.big_blind} · Buy-in {buyIn.toLocaleString()}
                      </div>
                    </div>
                    <div className="text-right">
                ...
                {short > 0 && (
                  <div className="mt-3 flex items-center justify-between gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2">
                    <div className="flex items-center gap-2 text-xs text-red-200">
                      ...
                      Need <span className="font-semibold">+{short.toLocaleString()}</span> chips to buy in
                    </div>
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={(e) => { e.preventDefault(); setDepositAmount(String(Math.max(50, Math.ceil(short / 50) * 50))); setCashierOpen(true); }}
                    >
                      Top up
                    </Button>
                  </div>
                  ...
                  <div key={t.id} className="card-felt shadow-card rounded-2xl p-4">
                    <div className="mb-2 flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          Host: {t.host_name} · Ended {t.ended_at ? new Date(t.ended_at).toLocaleString() : ""}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {meId.data === t.host_id && (
                        ...
                        <Link to="/play/$id" params={{ id: t.id }} className="text-xs text-gold hover:underline">View</Link>
                      </div>
                    </div>
                    {nets.length > 0 && (
                      <div className="mb-2 grid gap-1 text-sm">
                        {nets.sort((a, b) => (b.net ?? 0) - (a.net ?? 0)).map((n, i) => (
                          <div key={i} className="flex items-center justify-between rounded border border-border/40 bg-background/30 px-2 py-1">
                            <span className="truncate">{n.name ?? "Player"}</span>
                            <span className={n.net > 0 ? "text-emerald-400" : n.net < 0 ? "text-red-400" : "text-muted-foreground"}>
                              {n.net > 0 ? "+" : ""}{Number(n.net).toLocaleString()}
                            </span>
                          </div>
                        ))}
                      </div>
                    ...
                    {transfers.length > 0 && (
                      <div className="grid gap-1 text-xs text-muted-foreground">
                        ...
                        {transfers.map((tr, i) => (
                          <div key={i}>
                            {tr.from_name ?? "Player"} → {tr.to_name ?? "Player"}: <span className="text-foreground">{Number(tr.amount).toLocaleString()}</span>
                          </div>
                        ))}
                      </div>
                    )}
                    {nets.length === 0 && <div className="text-xs text-muted-foreground">No results recorded.</div>}
                  </div>
                );
              })}
            </div>
                    ...
                    <div key={t.id} className="card-felt shadow-card rounded-2xl p-4">
                      ...
                      {nets.length > 0 ? (
                        <div className="grid gap-1 text-sm">
                          {nets.map((n) => (
                            <div key={n.user_id} className="flex items-center justify-between rounded border border-border/40 bg-background/30 px-2 py-1">
                          ...
                          <div className="mt-1 flex items-center justify-between rounded border border-amber-400/30 bg-amber-500/10 px-2 py-1">
                            <span className="truncate font-semibold text-amber-200">{t.host_name} (Dealer)</span>
                            <span className={dealerNet > 0 ? "text-emerald-400" : dealerNet < 0 ? "text-red-400" : "text-muted-foreground"}>
