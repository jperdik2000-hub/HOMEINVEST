...
function NewNight() {
  ...
  const [date, setDate] = useState(() => {
    const d = new Date(); d.setDate(d.getDate() + 3); d.setHours(20, 0, 0, 0);
    return d;
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const v = schema.parse({
        title, starts_at: startsAt, location: locationName, buy_in: Number(buyIn) || 0, currency, notes,
      });
      ...
      if (isTourney) {
        if (splitTotal(tSetup.payoutSplit) !== 100) throw new Error("Prize split must add up to 100%");
        if (tSetup.blindLevels.length === 0) throw new Error("Add at least one blind level");
      }

      const { data: night, error } = await supabase.from("poker_nights").insert({
      ...
      for (const id of invitedIds) {
        ...
        if (p) invites.push({ night_id: night.id, invited_user_id: p.id, invited_email: p.email, invited_name: p.name });
      }
      const extras = extraEmails.split(/[,\n;\s]+/).map((s) => s.trim()).filter((s) => /.+@.+\..+/.test(s));
      ...
      if (invites.length) {
        ...
        if (ie) {
          ...
          if (registeredIds.length) {
            sendInvitePushes({ data: { nightId: night.id, userIds: registeredIds, whenText } })
      ...
      {meCheck.isSuccess && !meCheck.data && (
        <div className="mx-auto max-w-2xl">
          <div className="card-felt shadow-card rounded-2xl p-6 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Only the club admin can create new games. Ask the admin to schedule the next one.</p>
          </div>
        </div>
      )}
      {meCheck.data && (
        ...
        <form onSubmit={submit} className="card-felt shadow-card space-y-4 rounded-2xl p-6">
          ...
          <div>
            ...
            <div className="mt-1 grid grid-cols-2 gap-2">
              ...
              >
                <Coins className="mr-1 h-4 w-4" />Cash game
              </Button>
              <Button
                type="button"
              ...
              >
                <Timer className="mr-1 h-4 w-4" />Tournament
              </Button>
            </div>
          ...
          <div>
            ...
            <Textarea id="notes" value={notes} onChange={(e) => setNotes(e.target.value)} maxLength={2000} placeholder="Bring cash, small blinds start at $1/$2…"/>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 grid gap-1 rounded-md border border-border/60 bg-background/30 p-2 max-h-48 overflow-auto">
              {(() => {
                ...
                if (list.length === 0) {
                  return <div className="p-2 text-sm text-muted-foreground">No other members yet.</div>;
                }
                return list.map((p) => (
                <label key={p.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 hover:bg-background/40">
                  <input type="checkbox" checked={invitedIds.has(p.id)} onChange={() => toggle(p.id)} />
          ...
          <div>
            ...
            <Textarea id="extra" value={extraEmails} onChange={(e) => setExtraEmails(e.target.value)} placeholder="alice@example.com, bob@example.com"/>
          </div>
