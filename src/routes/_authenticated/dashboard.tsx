...
function Dashboard() {
  ...
  return (
    <AppShell>
      <div className="animate-in fade-in duration-500 mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
      ...
      <div className="animate-in fade-in duration-500 card-felt shadow-card mb-4 rounded-2xl p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 lg:divide-x lg:divide-border/60">
          <div className="lg:pl-0">
            <div className="font-display text-3xl font-bold tabular-nums text-gold">
              {formatMoney(volume.total)}
            </div>
            <div className="text-xs text-muted-foreground">Total buy-ins + rebuys</div>
          </div>
          <div className="lg:pl-4">
            <div className="font-display text-3xl font-bold tabular-nums">{volume.games}</div>
            <div className="text-xs text-muted-foreground">Completed games</div>
          </div>
          <div className="lg:pl-4">
            <div className="font-display text-3xl font-bold tabular-nums">
              {formatMoney(volume.avg)}
            </div>
            <div className="text-xs text-muted-foreground">Average per game</div>
          </div>
          <div className="lg:pl-4">
            <div className="font-display text-3xl font-bold tabular-nums">
              {formatMoney(volume.perEntry)}
            </div>
      ...
      <div className="animate-in fade-in duration-500 grid gap-4 lg:grid-cols-3">
              ...
              {upcoming.map((n) => (
                <li key={n.id}>
                  <Link
                    to="/nights/$id"
                    params={{ id: n.id }}
                    className="block rounded-lg border border-border/60 bg-background/40 p-3 transition-colors duration-200 hover:border-gold/40 hover:bg-background/60"
                  >
        ...
        <Card title="Top of the leaderboard" icon={<Trophy className="h-5 w-5 text-gold" />}>
          {leaderboard.length === 0 ? (
            <Empty text="No results yet. Log a game to seed the leaderboard." />
          ) : (
            <ol className="space-y-1">
              {leaderboard.slice(0, 3).map((r, i) => (
                <li
                  key={(r.user_id ?? r.name) + i}
                  className="flex items-center justify-between rounded-md bg-background/30 px-3 py-2 text-sm transition-colors duration-200 hover:bg-background/50"
                >
                  <span className="flex items-center gap-2">
                    <span className="w-5 font-display text-gold">#{i + 1}</span>
        ...
        <Card title="Recent games" icon={<Users className="h-5 w-5 text-gold" />}>
          {recent.length === 0 ? (
            <Empty text="Completed games will show up here." />
          ) : (
            <ul className="space-y-2">
              {recent.map((n) => (
                <li key={n.id}>
                  <Link
                    to="/nights/$id"
                    params={{ id: n.id }}
                    className="block rounded-md bg-background/30 px-3 py-2 text-sm transition-colors duration-200 hover:bg-background/50"
                  >
                    <div className="font-medium">{n.title}</div>
...
function Countdown({ nextNight }: { nextNight?: import("@/lib/poker").Night }) {
  const target = useMemo(
    () => (nextNight ? new Date(nextNight.starts_at).getTime() : 0),
    [nextNight],
  );
  ...
  return (
    <div className="card-felt shadow-card animate-in fade-in duration-500 mx-auto mt-8 max-w-3xl rounded-2xl p-6 text-center">
      ...
      <div className="grid grid-cols-4 gap-2 md:gap-4">
        {units.map((unit) => (
          <div key={unit.label} className="flex flex-col items-center">
            <div className="font-display text-4xl font-bold tabular-nums text-gold drop-shadow-[0_0_16px_oklch(0.80_0.14_85_/_0.35)] md:text-6xl">
              {pad(unit.value)}
            </div>
