...
import { Scale, ArrowUpRight, ArrowDownRight, Check, AlertTriangle, Lock, Bell } from "lucide-react";
import {
  listMySettlements,
  markSettlementPaid,
...
function statusTone(s: string) {
  switch (s) {
    case "unpaid": return "bg-red-500/15 text-red-300";
    case "payment_marked_sent": return "bg-amber-500/15 text-amber-300";
    case "payment_confirmed":
    case "fully_withdrawn": return "bg-emerald-500/15 text-emerald-300";
    case "partially_withdrawn": return "bg-sky-500/15 text-sky-300";
    case "disputed": return "bg-red-500/25 text-red-200";
...
function SettlementsPage() {
  ...
  const [payDialog, setPayDialog] = useState<null | { id: string; amount: number; toName: string }>(null);
  const [payMethod, setPayMethod] = useState("");
  const [payNote, setPayNote] = useState("");
  const [disputeDialog, setDisputeDialog] = useState<null | { id: string; fromName: string; amount: number }>(null);
  ...
  const markPaidMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setPayDialog(null); setPayMethod(""); setPayNote("");
    },
    onError: (e: any) => toast.error(e?.message ?? "Failed"),
  });

  const confirmMut = useMutation({
    mutationFn: (id: string) => confirmRecv({ data: { settlement_id: id } }) as Promise<{ ok: true }>,
    onSuccess: () => {
  ...
  const closeMut = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(r.created === 0 ? "Session balanced — no debts." : `Created ${r.created} settlement${r.created === 1 ? "" : "s"}.`);
  ...
  const remindMut = useMutation({
    ...
    onSuccess: (r: any) => {
      if (r?.noSubs) toast.info("Reminder sent — they haven't enabled push yet, but it's in their inbox.");
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground">
            External money owed between players. Payments happen off-platform — this page just tracks status.
          </p>
        </div>
          ...
          <section className="mb-8">
            ...
            <div className="card-felt shadow-card overflow-hidden rounded-2xl">
              {admin.data.tables.map((t: any) => (
                <div key={`${t.kind}-${t.id}`} className="flex items-center justify-between gap-3 border-b border-border/30 px-4 py-2 last:border-0 text-sm">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Closing a table locks results and generates the minimum set of debts. All players must have cashed out first.
            </p>
          </section>
        )}
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <section>
            <h2 className="mb-2 flex items-center gap-2 font-display text-lg font-semibold text-red-300">
              ...
              {unpaidOwed.length > 0 && (
                <span className="ml-1 flex h-2 w-2 rounded-full bg-red-500 animate-pulse" aria-label={`${unpaidOwed.length} unpaid debt${unpaidOwed.length === 1 ? "" : "s"}`} />
              )}
            </h2>
                ...
                return (
                  <div key={s.id} className="card-felt shadow-card rounded-2xl p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="text-xs uppercase tracking-wide text-muted-foreground">{s.session_name}</div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
                          <span className={`rounded-full px-2 py-0.5 ${statusTone(s.status)}`}>{STATUS_LABEL[s.status]}</span>
                          {s.marked_paid_at && <span className="text-muted-foreground">sent {fmtDate(s.marked_paid_at)}</span>}
                        </div>
                      </div>
                      {(s.status === "unpaid" || s.status === "disputed") && (
                        <Button
                          ...
                          variant="outline"
                          onClick={() => setPayDialog({ id: s.id, amount: Number(s.amount), toName })}
                        >
                          Mark as paid
                        </Button>
                      )}
                    ...
                    {s.payment_note && (
                      <div className="mt-2 text-[11px] text-muted-foreground">Note: {s.payment_note}</div>
                    )}
                  </div>
                );
              })}
          ...
          <section>
            ...
            <div className="space-y-2">
              ...
              {youAreOwed.map((s) => {
                ...
                return (
                  <div key={s.id} id={`s-${s.id}`} className="card-felt shadow-card rounded-2xl p-4 scroll-mt-24 target:ring-2 target:ring-gold/60">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="text-xs uppercase tracking-wide text-muted-foreground">{s.session_name}</div>
                        <div className="mt-1 text-sm">
                          <PlayerLink userId={fromId} name={fromName} className="font-semibold" /> owes you{" "}
                          <span className="font-mono text-gold">{fmt(s.amount)}</span>
                        </div>
                        <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
                          <span className={`rounded-full px-2 py-0.5 ${statusTone(s.status)}`}>{STATUS_LABEL[s.status]}</span>
                          {s.marked_paid_at && <span className="text-muted-foreground">sent {fmtDate(s.marked_paid_at)}</span>}
                          {s.confirmed_received_at && <span className="text-muted-foreground">received {fmtDate(s.confirmed_received_at)}</span>}
                        </div>
                        {s.dispute_reason && (
                      ...
                      {s.status === "payment_marked_sent" && (
                        <div className="flex flex-col gap-1">
                          ...
                          <Button
                            ...
                            onClick={() => setDisputeDialog({ id: s.id, fromName, amount: Number(s.amount) })}
                          >
                    ...
                    {s.payment_method && (
                      <div className="mt-2 text-[11px] text-muted-foreground">Method: {s.payment_method}</div>
                    )}
                    {s.payment_note && (
                      <div className="mt-1 text-[11px] text-muted-foreground">Note: {s.payment_note}</div>
                    )}
                  </div>
                );
              })}
        ...
        <p className="mt-6 text-xs text-muted-foreground">
          Confirming a payment marks it as received and unlocks that amount for virtual withdrawal from the Cashier.
        </p>
      </div>
      ...
      <Dialog open={!!payDialog} onOpenChange={(o) => !o && setPayDialog(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Confirm you paid <span className="font-semibold text-gold">{payDialog ? fmt(payDialog.amount) : ""}</span>{" "}
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input placeholder="Cash, Revolut, IRIS…" value={payMethod} onChange={(e) => setPayMethod(e.target.value)} />
            </div>
            <div>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => payDialog && markPaidMut.mutate({
      ...
      <Dialog open={!!disputeDialog} onOpenChange={(o) => !o && setDisputeDialog(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {disputeDialog && (
                <>Reject the claim that <span className="font-semibold">{disputeDialog.fromName}</span> sent{" "}
                <span className="font-semibold text-gold">{fmt(disputeDialog.amount)}</span>. Explain why so they can retry.</>
              )}
            </DialogDescription>
          </DialogHeader>
          <Textarea
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => disputeDialog && disputeMut.mutate({ settlement_id: disputeDialog.id, reason: disputeReason.trim() })}
            >
              Submit dispute
            </Button>
          </DialogFooter>
