...
function LogResults() {
  ...
  const existing = useQuery({
    ...
    queryFn: async () => (await supabase.from("player_results").select("*").eq("night_id", id)).data ?? [],
  });
  ...
  const entries = useQuery({
    ...
    enabled: isTourney,
    queryFn: async () =>
      ((await supabase.from("tournament_entries").select("*").eq("night_id", id)).data ?? []) as unknown as TournamentEntry[],
  });

  useEffect(() => {
    if (existing.data && rows.length === 0) {
      if (existing.data.length) {
        setRows(existing.data.map((r: any) => ({
          key: r.id,
          user_id: r.user_id,
        ...
        setRows(
          ordered.map((e) => ({
            ...
            rebuys: String((e.rebuys || 0) * Number(n?.rebuy_amount || 0) + (e.addons || 0) * Number(n?.addon_amount || 0)),
  ...
  function blankRow(defaultBuy: number): Row {
    return { key: crypto.randomUUID(), user_id: null, player_name: "", buy_in: String(defaultBuy), rebuys: "0", cash_out: "0", award: "" };
  }
  ...
  const totals = rows.reduce((s, r) => s + ((Number(r.cash_out) || 0) - ((Number(r.buy_in) || 0) + (Number(r.rebuys) || 0))), 0);
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="card-felt shadow-card rounded-2xl p-5">
            ...
            {rows.map((r, i) => (
              <div key={r.key} className="rounded-lg border border-border/60 bg-background/30 p-3">
                    ...
                    <select className="w-full rounded-md border border-input bg-background px-2 py-2 text-sm"
                      ...
                      onChange={(e) => {
                        ...
                        if (v === "__custom") update(i, { user_id: null });
                          ...
                          update(i, { user_id: v, player_name: p ? formatDisplayName(p.name, p.nickname) : r.player_name });
                        }
                      }}>
                      <option value="__custom">Custom name…</option>
                      {(profiles.data ?? []).map((p) => (
                        <option key={p.id} value={p.id}>{formatDisplayName(p.name, p.nickname)}</option>
                      ))}
                    </select>
                    {r.user_id === null && (
                      <Input className="mt-1" placeholder="Name" value={r.player_name} onChange={(e) => update(i, { player_name: e.target.value })}/>
                    )}
                  </div>
                  <div>
                    <Label className="text-xs">Buy-in</Label>
                    <Input type="number" value={r.buy_in} onChange={(e) => update(i, { buy_in: e.target.value })}/>
                  </div>
                  <div>
                    <Label className="text-xs">Re-buys</Label>
                    <Input type="number" value={r.rebuys} onChange={(e) => update(i, { rebuys: e.target.value })}/>
                  </div>
                  <div>
                    <Label className="text-xs">Cash-out</Label>
                    <Input type="number" value={r.cash_out} onChange={(e) => update(i, { cash_out: e.target.value })}/>
                  </div>
                  <div>
                    <Label className="text-xs">Award</Label>
                    <select className="w-full rounded-md border border-input bg-background px-2 py-2 text-sm"
                      value={r.award} onChange={(e) => update(i, { award: e.target.value })}>
                      <option value="">—</option>
                ...
                <div className="mt-2 text-right text-xs text-muted-foreground">
                  {isTourney && r.place != null && <span className="mr-2 text-gold">Finished #{r.place}</span>}
                  Net: <span className={((Number(r.cash_out) || 0) - ((Number(r.buy_in) || 0) + (Number(r.rebuys) || 0))) >= 0 ? "text-emerald-400" : "text-red-400"}>
                    {formatMoney((Number(r.cash_out) || 0) - ((Number(r.buy_in) || 0) + (Number(r.rebuys) || 0)), currency)}
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="mt-4 flex items-center justify-between">
            <Button type="button" variant="outline" onClick={() => setRows((rs) => [...rs, blankRow(Number(night.data?.buy_in ?? 0))])}>
              <Plus className="mr-1 h-4 w-4"/>Add player
            </Button>
            <div className="text-sm text-muted-foreground">
              Table balance: <span className={totals === 0 ? "text-emerald-400" : "text-amber-400"}>{formatMoney(totals, currency)}</span>
