      ...
      <form
        onSubmit={(e) => {
          ...
          if (pw.trim() === CASINO_PW) {
            try { localStorage.setItem(CASINO_UNLOCK_KEY, "1"); } catch {}
            onUnlock();
          } else {
            setErr(true);
          }
        }}
        className="card-felt shadow-card rounded-2xl p-6 w-full max-w-sm space-y-4"
      >
        ...
        <div className="text-center">
          <h1 className="font-display text-xl font-bold text-gold">Casino Entry</h1>
        ...
        <Button type="submit" className="w-full bg-gold shadow-gold">Enter</Button>
        <Link to="/dashboard" className="block text-center text-xs text-muted-foreground hover:text-gold">
          Back to Home
        </Link>
      </form>
    </div>
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const { data: prof } = await supabase
        .from("profiles").select("avatar_url").eq("id", u.id).maybeSingle();
      const raw = prof?.avatar_url ?? "";
      if (!raw) return;
      if (/^https?:\/\//i.test(raw)) { setAvatar(raw); return; }
      const { data: signed } = await supabase.storage
        .from("avatars").createSignedUrl(raw, 60 * 60 * 24 * 7);
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-52">
        <DropdownMenuItem asChild>
          <Link to="/play/account"><Wallet className="mr-2 h-4 w-4" /> Casino Account</Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link to="/play/settlements"><Scale className="mr-2 h-4 w-4" /> Settlements</Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link to="/play" hash="cashier"><Coins className="mr-2 h-4 w-4" /> Cashier</Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
export function CasinoShell({ children, compact }: { children: ReactNode; compact?: boolean }) {
  ...
  return (
    <div className={compact ? "h-[100dvh] flex flex-col overflow-hidden bg-[radial-gradient(ellipse_at_top,hsl(var(--felt)/0.35),transparent_60%),radial-gradient(ellipse_at_bottom,hsl(var(--gold)/0.08),transparent_60%)]" : "min-h-screen bg-[radial-gradient(ellipse_at_top,hsl(var(--felt)/0.35),transparent_60%),radial-gradient(ellipse_at_bottom,hsl(var(--gold)/0.08),transparent_60%)]"}>
      {!compact && (
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <Link to="/play" className="flex items-center gap-2">
            ...
            <div className="leading-tight">
              ...
              <div className="text-[10px] uppercase tracking-widest text-muted-foreground">High-Stakes Room</div>
            </div>
          </Link>
          <div className="flex items-center gap-1">
            <Link to="/dashboard" className="flex items-center gap-2 rounded-full border border-gold/40 bg-background/60 px-3 py-1 text-sm text-gold hover:bg-gold/10">
      ...
      <main className={compact ? "flex-1 min-h-0 flex flex-col" : "mx-auto max-w-6xl px-4 py-6"}>{children}</main>
    </div>
  );
}